...
namespace SMSAlarmSystem.Controllers
{
    ...
    public class AlarmPointsController : Controller
    {
        ...
        // 페이지네이션 처리를 위한 헬퍼
        private readonly PaginationHelper<AlarmPoint> _paginationHelper;
        ...
        public AlarmPointsController(
            ...
            PaginationHelper<AlarmPoint> paginationHelper,
        ...
        /// <param name="pageSize">페이지당 표시할 알람 포인트 수</param>
        /// <returns>페이지네이션이 적용된 알람 포인트 목록 뷰</returns>
        public async Task<IActionResult> Index(int page = 1, int pageSize = 10)
        {
            try
            {
                // 페이지네이션 파라미터 로깅
                _logger.LogInformation("알람 포인트 목록 조회 시작: 페이지={Page}, 페이지크기={PageSize}", page, pageSize);

                // 모든 알람 포인트 조회
                var allAlarmPoints = await _alarmService.GetAllAlarmPointsAsync();

                // 페이지네이션 헬퍼를 사용하여 페이지네이션 적용
                var model = _paginationHelper.CreatePaginatedModel(allAlarmPoints, page, pageSize);

                // 결과 로깅
                _logger.LogInformation("알람 포인트 목록 조회 완료: 총 {TotalItems}개 중 {PagedCount}개 표시",
            ...
            catch (Exception ex)
            {
                ...
                // 오류 발생 시 빈 모델로 뷰 표시
                return View(new PaginatedViewModel<AlarmPoint>());
            }
        }
